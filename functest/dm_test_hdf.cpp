...
TEST(dm_hdf_test, from_xengraph_slow1) {
    ...
    run_preprocessor("--method dm --input-format xengraph --output-format hdf --preprocessing-mode slow --input-file functest/01_xengraph.xeng --output-file from_xengraph_slow1");
    check_hdf("from_xengraph_slow1.hdf5", "functest/01_dm.csv");
}

TEST(dm_hdf_test, from_xengraph_slow2) {
    // slow with precision loss
    run_preprocessor("--method dm --input-format xengraph --output-format hdf --preprocessing-mode slow --input-file functest/02_xengraph.xeng --output-file from_xengraph_slow2 --precision-loss 100");
...
TEST(dm_hdf_test, from_dimacs_slow2) {
    // slow with precision loss
    run_preprocessor("--method dm --input-format dimacs --output-format hdf --preprocessing-mode slow --input-file functest/02_dimacs.gr --output-file from_dimacs_slow2 --precision-loss 100");
...
TEST(dm_hdf_test, from_csv_slow2) {
    // slow with precision loss
    run_preprocessor("--method dm --input-format csv --output-format hdf --preprocessing-mode slow --input-file functest/02_adj.csv --output-file from_csv_slow2 --precision-loss 100");
