...
struct NodeWeight {
	...
	NodeWeight(NodeID_ v) :
		v(v),
		w(1),
		t(1) {
	}

	NodeWeight(NodeID_ v, WeightT_ w) :
		v(v),
		w(w),
		t(1) {
	}

	NodeWeight(NodeID_ v, WeightT_ w, TimestampT_ t) :
		v(v),
		w(w),
		t(t) {
	}
...
public:
	CSRGraph() :
		...
		edges_(nullptr),
		vertices_(nullptr),
		segment_edges_actual(nullptr),
		segment_edges_total(nullptr) {
	}
	...
	CSRGraph(EdgeList& edge_list, bool is_directed, int64_t n_edges, int64_t n_vertices) {
		...
		if(debug) cout << ">>>>>>>> root-density: " << ((double) segment_edges_actual[1] / (double) segment_edges_total[
			1]) << endl;
	}
	...
	void PrintStats() const {
		std::cout << "Graph has " << num_vertices << " nodes and " << num_edges_ << " ";
		if(!directed_) std::cout << "un";
	...
	pvector<SGOffset> VertexOffsets(bool in_graph = false) const {
		...
		for(NodeID_ n = 0; n < num_vertices + 1; n++) offsets[n] = vertices_[n].index - vertices_[0].index;
		return offsets;
	}

	Range<NodeID_> vertices() const {
		return Range<NodeID_>(get_node_count());
	}
	...
	inline void print_pma_meta() {
		...
		cout << "segment_count: " << segment_count << ", segment_size: " << segment_size << ", tree_height: " <<
			tree_height << endl;
	}
	...
	inline void edge_list_boundary_sanity_checker() {
		for(int32_t curr_vertex = 1; curr_vertex < num_vertices; curr_vertex += 1) {
			if(vertices_[curr_vertex - 1].index + vertices_[curr_vertex - 1].degree > vertices_[curr_vertex].index) {
				...
				cout << " degree: " << vertices_[curr_vertex - 1].degree << " next vertex start at: " << vertices_[
					curr_vertex].index << endl;
			}
			assert(
				vertices_[curr_vertex - 1].index + vertices_[curr_vertex - 1].degree <= vertices_[curr_vertex].index &&
				"Invalid edge-list boundary found!"
			);
		}
		assert(
			vertices_[num_vertices - 1].index + vertices_[num_vertices - 1].degree <= elem_capacity &&
			"Invalid edge-list boundary found!"
		);
	}
	...
	void recount_segment_total() {
		...
		for(int i = 0; i < segment_count; i++) {
			int64_t next_starter = (i == (segment_count - 1))
				                       ? (elem_capacity)
				                       : vertices_[(i + 1) * segment_size].index;
	...
	void segment_sanity_check() {
		for(int i = 0; i < segment_count; i++) {
			...
			while(j > 0) {
				assert(
					segment_edges_total[j] >= segment_edges_actual[j] &&
					"Rebalancing (adaptive) segment boundary invalidate segment capacity!"
				);
				j /= 2;
			}
		}
	...
	void insert(int32_t src, int32_t dst, int32_t value) {
		...
		if(right_free_slot == -1 && right_vertex_boundary == (num_vertices - 1) && elem_capacity > (1 + vertices_[
			num_vertices - 1].index + vertices_[num_vertices - 1].degree)) {
	...
	void spread_weighted(int64_t from, int64_t to, int64_t n_edges, int32_t start_vertex, int32_t end_vertex) {
	...
	void spread_weighted_V1(int32_t start_vertex, int32_t end_vertex) {
		...
		if(debug) {
			for(int32_t curr_vertex = start_vertex; curr_vertex < end_vertex; curr_vertex += 1) {
				...
				cout << ", degree: " << vertices_[curr_vertex].degree << ", new position: " << new_positions[
					curr_vertex] << endl;
			}
		}
	...
	void rebalance_wrapper(int32_t src) {
		...
		if(debug) cout << "Window: " << window << ", density: " << density << ", up_height: " << up_height <<
			", low_height: " << low_height << endl;
	...
	int64_t* calculate_positions_V1(int32_t start_vertex, int32_t end_vertex, int64_t gaps, int64_t total_degree) {
		...
		for(int i = start_vertex; i < end_vertex; i++) {
			...
			if(i > start_vertex) {
				...
				assert(
					new_index[i-start_vertex] >= new_index[(i-1)-start_vertex] + vertices_[i-1].degree &&
	...
	void rebalance_weighted(int32_t start_vertex, int32_t end_vertex, int32_t pma_idx) {
		...
		assert(
			new_index[size - 1] + vertices_[end_vertex - 1].degree <= index_boundary &&
