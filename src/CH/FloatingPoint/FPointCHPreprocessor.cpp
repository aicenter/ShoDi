...
#include "FPointCHPreprocessor.h"
#include "../../Timer/Timer.h"
#include "../Generic/EdgeDifferenceManager.h"
#include "Structures/FPointHopsDijkstraNode.h"
#include <climits>
#include <queue>

// Initialization only
//______________________________________________________________________________________________________________________
vector<bool> FPointCHPreprocessor::contracted(0);
vector<unsigned int> FPointCHPreprocessor::preprocessingDegrees(0);
vector<double> FPointCHPreprocessor::dijkstraDistance(0);
vector<FPointShortcutEdge> FPointCHPreprocessor::allShortcuts;
unordered_map<pair<unsigned int, unsigned int>, double, FPointCHPreprocessor::pair_hash> FPointCHPreprocessor::distances;
unordered_map<pair<unsigned int, unsigned int>, double, FPointCHPreprocessor::pair_hash> FPointCHPreprocessor::distancesWithoutX;
vector<unsigned int> FPointCHPreprocessor::sources(0);
vector<unsigned int> FPointCHPreprocessor::targets(0);
unordered_set<unsigned int> FPointCHPreprocessor::targetsSet;
unordered_map<unsigned int, vector<pair<unsigned int, double > > > FPointCHPreprocessor::buckets;

// The 'main' preprocessing function. The result of calling this function is the FPointUpdateableGraph containing all the
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::preprocessForDDSGF(FPointUpdateableGraph &graph) {
    ...
    FPointCHPreprocessor::contracted.resize(graph.nodes(), false);
    FPointCHPreprocessor::preprocessingDegrees.resize(graph.nodes());
    FPointCHPreprocessor::dijkstraDistance.resize(graph.nodes(), numeric_limits<double>::max());
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::initializePriorityQueue(CHpriorityQueue & priorityQueue, FPointUpdateableGraph & graph) {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::contractNodesWithUnpackingData(CHpriorityQueue &priorityQueue, FPointUpdateableGraph &graph) {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::reinsertShortcuts(FPointUpdateableGraph & graph) {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::adjustNeighbourgDegrees(const unsigned int x, FPointUpdateableGraph & graph) {
    unordered_map < unsigned int, double > previousNodes = graph.incomingEdges(x);
    unordered_map < unsigned int, FPointPreprocessingEdgeData > nextNodes = graph.outgoingEdges(x);
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::updateNeighboursPriorities(const unsigned int x, FPointUpdateableGraph & graph, CHpriorityQueue & priorityQueue) {
    unordered_map < unsigned int, double > previousNodes = graph.incomingEdges(x);
    unordered_map < unsigned int, FPointPreprocessingEdgeData > nextNodes = graph.outgoingEdges(x);
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::getPossibleShortcuts(const unsigned int i, FPointUpdateableGraph & graph, bool deep) {
    getDistancesUsingNode(i, graph);
    FPointCHPreprocessor::contracted[i] = true;
    manyToManyWithBuckets(graph, deep);
    FPointCHPreprocessor::contracted[i] = false;
}
...
// to many search.
//______________________________________________________________________________________________________________________
unsigned int FPointCHPreprocessor::calculateShortcutsAmount() {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::actuallyAddShortcutsWithUnpackingData(FPointUpdateableGraph & graph, unsigned int x) {
    for(auto iter1 = sources.begin(); iter1 != sources.end(); ++iter1) {
        for(auto iter2 = targets.begin(); iter2 != targets.end(); ++iter2) {
            if(*iter1 != *iter2) {
                if (distancesWithoutX.at(make_pair(*iter1, *iter2)) > distances.at(make_pair(*iter1, *iter2))) {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::removeContractedNodeEdges(FPointUpdateableGraph & graph, unsigned int x) {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::clearStructures() {
    distances.clear();
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::getDistancesUsingNode(const unsigned int i, FPointUpdateableGraph & graph ) {
    unordered_map <unsigned int, double> i_sources = graph.incomingEdges(i);
    unordered_map <unsigned int, FPointPreprocessingEdgeData> i_targets = graph.outgoingEdges(i);
...
//______________________________________________________________________________________________________________________
double FPointCHPreprocessor::longestPossibleShortcut(const unsigned int source) {
    double longest = 0;
    for(auto iter = targets.begin(); iter != targets.end(); ++iter) {
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::manyToManyWithBuckets(FPointUpdateableGraph & graph, bool deep) {
    double lowestBucketVal = numeric_limits<double>::max();
    ...
    for(unsigned int i = 0; i < sources.size(); i++) {
        double longestShortcut = longestPossibleShortcut(sources[i]);
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::initBuckets(const unsigned int x, FPointUpdateableGraph & graph, double & lowestBucketVal) {
    const unordered_map<unsigned int, double> neighbours = graph.incomingEdges(x);
    for(auto iter = neighbours.begin(); iter != neighbours.end(); ++iter) {
        if (! contracted[(*iter).first]) {
            if (buckets.count((*iter).first) == 0) {
                buckets.insert(make_pair((*iter).first, vector<pair<unsigned int, double>>()));
            }
...
//______________________________________________________________________________________________________________________
void FPointCHPreprocessor::oneToManyWithBuckets(const unsigned int source, const double upperBound, FPointUpdateableGraph & graph, unsigned int hoplimit, unsigned int maxexpanded) {
    ...
    auto cmp = [](FPointHopsDijkstraNode left, FPointHopsDijkstraNode right) { return (left.weight) > (right.weight);};
    priority_queue<FPointHopsDijkstraNode, vector<FPointHopsDijkstraNode>, decltype(cmp)> q(cmp);
    q.push(FPointHopsDijkstraNode(source, 0, 0));

    while(! q.empty() ) {
        const FPointHopsDijkstraNode current = q.top();
        expanded++;
        ...
        const unordered_map < unsigned int, FPointPreprocessingEdgeData > & neighbours = graph.outgoingEdges(current.ID);
        for ( auto n_iter = neighbours.begin(); n_iter != neighbours.end(); ++n_iter ) {
            if (buckets.count((*n_iter).first) == 1 ) {
                for(auto iter = buckets[(*n_iter).first].begin(); iter != buckets[(*n_iter).first].end(); ++iter) {
                    double newDistUsingBucket = current.weight + (*n_iter).second.weight + (*iter).second;
            ...
            double newDistance = current.weight + (*n_iter).second.weight;
            if ( ! FPointCHPreprocessor::contracted[(*n_iter).first] && newDistance < dijkstraDistance[(*n_iter).first]) {
                dijkstraDistance[(*n_iter).first] = newDistance;
                nodesWithChangedDistances.push_back((*n_iter).first);
                q.push(FPointHopsDijkstraNode((*n_iter).first, newDistance, current.hops + 1));
            }
        }
    ...
    for(unsigned int i = 0; i < nodesWithChangedDistances.size(); i++) {
        dijkstraDistance[nodesWithChangedDistances[i]] = numeric_limits<double>::max();
    }
