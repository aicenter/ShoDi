...
graph_t* johnson::johnson_init(std::vector<int> adj_matrix) {
	size_t E = 0;
	for (size_t i = 0; i < adj_matrix.size(); i++) {
		if (adj_matrix[i] < INT_MAX) {
			E++;
		}
	}

	int n = sqrt(adj_matrix.size());
...
graph_t *johnson::johnson_init2(const int n, const double p, const unsigned long seed) {
...
void johnson::johnson_parallel(graph_t* gr, int* output) {
