...
public:
    ...
    static double runAndMeasureFlagsGraphOutputAndRetval(const vector < pair < unsigned int, unsigned int> > & trips, const FlagsGraph & graph, vector < unsigned int > & ranks, vector < long long unsigned int > & distances);
};
